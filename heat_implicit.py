...
mainDiag = (1. + 2. * alpha) * np.ones(int(N - 1))
...
mat = np.array([np.insert(upperDiag, 0, 0), mainDiag, np.insert(lowerDiag, int(N), 0)])
for k in range(0, int(steps)):
    u[k + 1] = linalg.solve_banded((1, 1), mat, u[k])
...
plt.colorbar()
plt.tight_layout()
plt.savefig('plots/implicit_heat.pdf')
